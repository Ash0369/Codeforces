...
{
    public:
    vector<int>rank,parent,size,vacant;
    dsu(int n)
    {
...
int main()
{
   ...
   // cin>>tt;
    for(int tc=0;tc<tt;tc++)
    {
        ...
        for(int i=0;i<n;i++)
        {
            if(ds.parent[vec[i]]==vec[i])
            {
                cout<<vec[i]<<" ";
                ds.parent[vec[i]]=max(1,(vec[i]+1)%(n+1));
            }
            else
            {
                int p=ds.find_parent(vec[i]);
                cout<<ds.find_parent(vec[i])<<" ";
                ds.parent[p]=max(1,(p+1)%(n+1));
            }
        }
        cout<<endl;
