   ...
   // cin>>tt;
    for(int tc=0;tc<tt;tc++)
    {
        ...
        for(int i=0;i<vec.size();i++)
        {
            ...
            //Kepping i node as minimum weight

            int cc=1;

            for(int j=i;j<vec.size() && cc<n;j++)
            {
                if(ds.find_parent(vec[j][1])!=ds.find_parent(vec[j][2]))
                {
                    ds.union_by_rank(vec[j][1],vec[j][2]);
                    cc++;
                }
                if(cc==n)
                {
                    ans=min(ans,vec[j][0]-vec[i][0]);
                }
            }
        }
        if(ans==1e18)
